...
void mpu_reset(void) { mpu_centralize_angles(); }

static void angles_upd(uint32_t ms_from_last_measure, mpud::float_axes_t *old_angles,
                       mpud::float_axes_t *gyro_data)
{
...
static void mpu_task(void *)
{
    ...
    while (1) {
        ...
        printf("[ %d\t%d\t%d\t%d ]\n", (int8_t) REGR(REG_ANGLE_X), (int8_t) REGR(REG_ANGLE_Y),
               (uint8_t) REGR(REG_ANGLE_Z_SIGN), (uint8_t) REGR(REG_ANGLE_Z));
#endif
