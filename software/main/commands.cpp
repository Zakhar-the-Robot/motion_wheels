...
static void precise_position(uint8_t current_angle, uint8_t target_angle, bool current_sign)
{
    ...
    {
        if (current_angle < target_angle) motor_ctrl.Cmd(MotorCmd_Left, 1000);
        if (current_angle > target_angle) motor_ctrl.Cmd(MotorCmd_Right, 1000);
    } else {  // positive angle
        if (current_angle < target_angle) motor_ctrl.Cmd(MotorCmd_Right, 1000);
        if (current_angle > target_angle) motor_ctrl.Cmd(MotorCmd_Left, 1000);
    }
...
static void set_position(MotorControllerCmd_t rot_direction, int delta_angle)
{
    ...
    uint8_t target_sign;
    if (rot_direction == MotorCmd_Left) { delta_angle = -delta_angle; }
...
#if ENABLE_POSITION_UNIT
    if (regs_get_arg()) { set_position(MotorCmd_Left, regs_get_arg()); }
#endif  // ENABLE_POSITION_UNIT
}
...
#if ENABLE_POSITION_UNIT
    if (regs_get_arg()) { set_position(MotorCmd_Right, regs_get_arg()); }
#endif  // ENABLE_POSITION_UNIT
}
